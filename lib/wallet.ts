...
const DAILY_LIMIT_CENTS = () => parseInt(process.env.WALLET_DAILY_LIMIT_CENTS || '500', 10);
const PER_CALL_LIMIT_CENTS = () => parseInt(process.env.WALLET_PER_CALL_LIMIT_CENTS || '200', 10);

function stripeRequest(path: string, { method = 'GET', form }: { method?: string; form?: Record<string, string> } = {}) {
  ...
  return fetch(url, {
    ...
    if (!r.ok) {
      const err = new Error((data as any)?.error?.message || `Stripe ${r.status}`);
      (err as any).stripe = (data as any)?.error;
...
async function resolvePaymentMethodId() {
  ...
  if (!pm) {
    throw new Error('No default payment method on customer. Set STRIPE_PAYMENT_METHOD_ID or attach one in Stripe.');
  }
...
async function listWalletPayments(hours = 24) {
  ...
  return ((data.data || []) as any[]).filter(
    (pi: any) => pi.metadata?.wallet === 'agent' && pi.status === 'succeeded'
  );
}

async function getDailySpendCents() {
  const payments = await listWalletPayments(24);
  return payments.reduce((sum: number, pi: any) => sum + (pi.amount_received || pi.amount || 0), 0);
}

export async function walletStatus() {
  const payments = await listWalletPayments(24);
  const spent = payments.reduce((sum: number, pi: any) => sum + (pi.amount_received || pi.amount || 0), 0);
...
export async function confirmPayment({ paymentIntentId, expectedAmountCents, metadata = {} }: {
  ...
  if (amountCents > PER_CALL_LIMIT_CENTS()) {
    const err = new Error(`Amount ${amountCents}¢ exceeds per-call limit ${PER_CALL_LIMIT_CENTS()}¢`);
    (err as any).code = 'PER_CALL_LIMIT_EXCEEDED';
    (err as any).amount_cents = amountCents;
    (err as any).per_call_limit_cents = PER_CALL_LIMIT_CENTS();
    throw err;
  }

  if (pi.status === 'succeeded') {
    const isWalletPaid = pi.metadata?.wallet === 'agent';
    return { payment_intent_id: pi.id, status: 'already_succeeded', amount_cents: amountCents, wallet_managed: isWalletPaid };
  }

  const spent = await getDailySpendCents();
  if (spent + amountCents > DAILY_LIMIT_CENTS()) {
    const err = new Error(`Charge of ${amountCents}¢ would exceed daily limit: ${spent}¢ already spent of ${DAILY_LIMIT_CENTS()}¢`);
    (err as any).code = 'DAILY_LIMIT_EXCEEDED';
    (err as any).daily_spend_cents = spent;
    (err as any).daily_limit_cents = DAILY_LIMIT_CENTS();
    (err as any).amount_cents = amountCents;
    throw err;
  }
  ...
  await stripeRequest(`/payment_intents/${paymentIntentId}`, { method: 'POST', form: updateForm });

  const confirmed = await stripeRequest(`/payment_intents/${paymentIntentId}/confirm`, {
    method: 'POST',
    form: { payment_method: pmId, off_session: 'true' },
  });

  if (confirmed.status !== 'succeeded') {
    const err = new Error(`Payment did not reach 'succeeded' (status=${confirmed.status})`);
    (err as any).code = 'PAYMENT_NOT_SUCCEEDED';
    (err as any).stripe_status = confirmed.status;
    (err as any).next_action = confirmed.next_action || null;
    throw err;
  }
...
export function mapWalletErrorToStatus(code: string) {
  switch (code) {
