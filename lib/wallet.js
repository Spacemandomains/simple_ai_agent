...
export async function confirmPayment({ paymentIntentId, expectedAmountCents, metadata = {} }) {
  ...
  // Stripe's /confirm endpoint does not accept `metadata`. Set metadata first
  // via an update call so listWalletPayments() can still tag this charge as
  // wallet-managed.
  const metadataForm = {
  ...
  for (const [k, v] of Object.entries(metadata)) {
    if (v !== undefined && v !== null) metadataForm[`metadata[${k}]`] = String(v);
  }
